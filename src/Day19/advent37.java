...
public class advent37 {
    ...
    public static void main(String[] args) throws IOException {
         ...
         * }
         */
        List<Ponto3D> output = new ArrayList<Ponto3D>();

        for (int i = 0; i < input.size(); i++) {
            int size = input.get(i).size();
            List<Ponto3D> temp = input.get(i);
            Ponto3D relativeSc = relativeScanners.get(i);
            Character[] p = positions.get(i);
            int[] s = signals.get(i);
            for (int j = 0; j < size; j++) {
                Ponto3D temp2 = relativeSc.customOrientationAdd(temp.get(j), p, s);
                if (!containsPonto3D(output, temp2)) {
                    output.add(temp2);
                }
            }
        }
